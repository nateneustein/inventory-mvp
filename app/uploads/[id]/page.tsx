...
export default async function UploadDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return (
    <>
      ...
      <div className="card table-card"><div className="table-head"><h2>Rows from this upload</h2><Link className="button small-btn secondary" href="/imported-orders">All imported rows</Link></div><div className="wide-table"><table><thead><tr><th>#</th><th>Order</th><th>SKU</th><th>Qty</th><th>Item</th><th>Variation</th><th>Mapping</th><th></th></tr></thead><tbody>{(rows || []).map((r:any)=><tr key={r.id}><td>{r.source_row_number}</td><td><Link className="link" href={`/imported-orders/${r.id}`}>{r.platform_order_id}</Link></td><td>{r.platform_sku}</td><td>{num(r.quantity)}</td><td>{r.item_name}</td><td>{r.variation_text}</td><td><span className={`badge ${r.mapping_status}`}>{r.mapping_status}</span></td><td><Link className="button small-btn secondary" href={`/imported-orders/${r.id}`}>Open</Link></td></tr>)}{(rows || []).length === 0 && <tr><td colSpan={8}><div className="empty-state">No rows found for this upload.</div></td></tr>}</tbody></table></div></div>
    </>
  )
}
