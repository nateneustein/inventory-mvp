    ...
    <>
      ...
      <div className="card table-card">
        ...
        <div className="wide-table"><table>
          <thead><tr><th>Source</th><th>Account</th><th>Order ID</th><th>Date</th><th>Week</th><th className="sku-col">SKU</th><th>Qty</th><th>Item</th><th>Variation</th><th>Customization</th><th>Status</th><th>Mapping</th><th>Actions</th></tr></thead>
          <tbody>
            {rows.map((r:any) => (
              <tr key={r.id}>
                <td>{r.platform}</td><td>{r.account_name}</td><td><Link className="link" href={`/imported-orders/${r.id}`}>{r.platform_order_id}</Link></td><td>{date(r.order_date_parsed || r.order_date)}</td><td>{date(r.week_start)}</td><td className="sku-col">{r.platform_sku}</td><td>{num(r.quantity)}</td><td>{r.item_name}</td><td>{r.variation_text}</td><td>{r.customization_text}</td><td>{r.order_status}</td><td><span className={`badge ${r.mapping_status}`}>{r.mapping_status}</span><br/><span className="small muted">{r.mapped?.internal_sku}</span></td>
                <td><details><summary className="button small-btn secondary">Edit</summary><form className="stack card flat" action={updateImportedOrderRow}><input type="hidden" name="id" value={r.id}/><label>Mapping status<select name="mapping_status" defaultValue={r.mapping_status}><option value="unmapped">Unmapped</option><option value="mapped">Mapped</option><option value="needs_review">Needs review</option><option value="ignored">Ignored</option></select></label><label>Actual variation<select name="mapped_variation_id" defaultValue={r.mapped_variation_id || ''}><option value="">None</option>{(variations || []).map((v:any)=><option key={v.id} value={v.id}>{v.products?.name} · {v.variation_name} · {v.internal_sku}</option>)}</select></label><label>Demand variation<select name="demand_variation_id" defaultValue={r.demand_variation_id || ''}><option value="">Same/none</option>{(variations || []).map((v:any)=><option key={v.id} value={v.id}>{v.products?.name} · {v.variation_name} · {v.internal_sku}</option>)}</select></label><button type="submit">Save mapping</button></form><form action={deleteImportedOrderRow}><input type="hidden" name="id" value={r.id}/><button className="danger small-btn" type="submit">Delete row</button></form></details></td>
              </tr>
            ))}
