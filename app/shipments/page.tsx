  ...
  return (
    <>
      ...
      <div className="grid two">
        <div className="card"><details className="add-panel"><summary className="button">+ Create shipment / PO</summary></details><form className="stack" action={createPurchaseOrder}><label>PO / shipment number<input name="po_number" required placeholder="PO-1001" /></label><label>Supplier<SearchSelect name="supplier_id" required placeholder="Type a supplier" options={(suppliers || []).map((s: any) => ({ value: s.id, label: s.name }))} /></label><div className="form-row"><label>Order date<input name="order_date" type="date" /></label><label>Expected arrival<input name="expected_date" type="date" /></label></div><label>Status<select name="status"><option value="ordered">Ordered</option><option value="in_production">In production</option><option value="shipped">Shipped</option><option value="delivered">Delivered</option></select></label><label>Tracking number<input name="tracking_number" /></label><label>Notes<textarea name="notes" /></label><button type="submit">Create shipment</button><button type="button" className="button secondary cancel-btn">Cancel</button></form></div>
        <div className="card"><details className="add-panel"><summary className="button">+ Add item to shipment</summary></details><form className="stack" action={addPurchaseOrderItem}><label>Shipment / PO<SearchSelect name="purchase_order_id" required placeholder="Type a PO number or supplier" options={(allPos || []).map((po: any) => ({ value: po.id, label: `${po.po_number} · ${po.suppliers?.name}` }))} /></label><label>Part<SearchSelect name="part_id" required placeholder="Type a part name or SKU" options={(parts || []).map((p: any) => ({ value: p.id, label: p.name, hint: p.sku }))} /></label><div className="form-row"><label>Qty ordered<input name="quantity_ordered" type="number" step="0.01" required /></label><label>Unit cost<input name="unit_cost" type="number" step="0.01" defaultValue="0" /></label></div><label>Notes<textarea name="notes" /></label><button type="submit">Add shipment item</button><button type="button" className="button secondary cancel-btn">Cancel</button></form></div>
      </div>
