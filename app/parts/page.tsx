  ...
  return (
    <>
      ...
      <div className="card table-card">
        ...
        <div className="wide-table compact-rows"><table><thead><tr><th>Part</th><th>SKU</th><th>Category</th><th>On hand</th><th>Incoming</th><th>Projected</th><th>Reorder point</th><th>Status</th><th className="actions-cell">Actions</th></tr></thead><tbody>{parts.map((p: any) => <tr key={p.part_id}><td><Link className="link" href={`/parts/${p.part_id}`}>{p.name}</Link></td><td className="sku-cell" title={p.sku}>{p.sku}</td><td>{p.category}</td><td>{num(p.on_hand)}</td><td>{num(p.incoming_qty)}</td><td>{num(p.projected_qty)}</td><td>{num(p.reorder_point)}</td><td><span className={`badge ${p.stock_status}`}>{p.stock_status}</span></td><td className="actions-cell"><div className="action-row"><Link className="button small-btn secondary" href={`/parts/${p.part_id}`}>Open</Link><Link className="button small-btn" href={`/predictions/advanced?part_id=${p.part_id}`}>Reorder</Link><form className="inline-form" action={archivePart}><input type="hidden" name="id" value={p.part_id} /><input type="hidden" name="active" value={p.active ? 'false' : 'true'} /><button className="small-btn ghost" type="submit">{p.active ? 'Archive' : 'Restore'}</button></form><form className="inline-form" action={deletePart}><input type="hidden" name="id" value={p.part_id} /><button className="small-btn danger" type="submit">Remove</button></form></div></td></tr>)}{parts.length === 0 && <tr><td colSpan={9}><div className="empty-state">No parts match this filter.</div></td></tr>}</tbody></table></div>
      </div>
    </>
  )
}
