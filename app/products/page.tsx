...
export default async function ProductsPage({ searchParams }: { searchParams?: Promise<{ q?: string, active?: string }> }) {
  ...
  return (
    <>
      ...
      <div className="card table-card">
        ...
        <table><thead><tr><th>Product</th><th className="sku-col">SKU</th><th>Status</th><th>Notes</th><th className="actions-cell">Actions</th></tr></thead><tbody>{products.map((p:any) => <tr key={p.id}><td><Link className="link" href={`/products/${p.id}`}>{p.name}</Link></td><td className="sku-col">{p.sku}</td><td><span className={`badge ${p.active ? 'ok' : 'archived'}`}>{p.active ? 'active' : 'archived'}</span></td><td>{p.notes}</td><td><div className="action-row"><Link className="button small-btn secondary" href={`/products/${p.id}`}>Open</Link><form action={archiveProduct}><input type="hidden" name="id" value={p.id} /><input type="hidden" name="active" value={p.active ? 'false' : 'true'} /><button className="small-btn ghost" type="submit">{p.active ? 'Archive' : 'Restore'}</button></form></div></td></tr>)}{products.length === 0 && <tr><td colSpan={5}><div className="empty-state">No products match this filter.</div></td></tr>}</tbody></table>
      </div>

      <div className="card table-card">
        <div className="table-head"><h2>Variations</h2><Link className="button small-btn secondary" href="/boms">BOMs</Link></div>
        <div className="wide-table"><table><thead><tr><th>Product</th><th>Variation</th><th className="sku-col">Internal SKU</th><th>Status</th><th>Notes</th><th>Actions</th></tr></thead><tbody>{variations.map((v:any) => <tr key={v.id}><td><span className="entity-name">{v.products?.name}</span></td><td><span className="entity-name">{v.variation_name}</span><br/><span className="sku-small">{v.internal_sku}</span></td><td className="sku-col">{v.internal_sku}</td><td><span className={`badge ${v.active ? 'ok' : 'archived'}`}>{v.active ? 'active' : 'archived'}</span></td><td>{v.notes}</td><td><div className="action-row"><Link className="button small-btn secondary" href={`/products/${v.product_id}`}>Open product</Link><form action={archiveVariation}><input type="hidden" name="id" value={v.id} /><input type="hidden" name="product_id" value={v.product_id} /><input type="hidden" name="active" value={v.active ? 'false' : 'true'} /><button className="small-btn ghost" type="submit">{v.active ? 'Archive' : 'Restore'}</button></form></div></td></tr>)}{variations.length === 0 && <tr><td colSpan={6}><div className="empty-state">No variations yet.</div></td></tr>}</tbody></table></div>
      </div>
    </>
  )
}
