    ...
    <>
      ...
      <div className="card">
        ...
        <form className="stack" action={createBomItem}>
          <div className="form-row"><label>Variation<SearchSelect name="variation_id" required placeholder="Type a variation" options={(variations || []).map((v: any) => ({ value: v.id, label: `${v.internal_sku} · ${v.variation_name}` }))} /></label><label>Part<SearchSelect name="part_id" required placeholder="Type a part name or SKU" options={(parts || []).map((p: any) => ({ value: p.id, label: `${p.sku} · ${p.name}` }))} /></label><label>Qty per unit<input name="quantity_per_unit" type="number" step="0.0001" required defaultValue="1" /></label></div>
          <label>Notes<textarea name="notes" /></label>
          <button type="submit">Add BOM item</button>
        </form>
      </div>

      <div className="card table-card"><div className="table-head"><h2>BOM recipe for this product</h2><Link className="button small-btn secondary" href="/boms">Full BOM sheet</Link></div><table><thead><tr><th>Variation</th><th>Part</th><th>Qty per unit</th><th>Notes</th><th>Actions</th></tr></thead><tbody>{(bomItems || []).map((b:any) => <tr key={b.id}><td>{b.product_variations?.internal_sku} · {b.product_variations?.variation_name}</td><td>{b.parts?.sku} · {b.parts?.name}</td><td>{num(b.quantity_per_unit)}</td><td>{b.notes}</td><td><details><summary className="button small-btn secondary">Edit</summary><form className="stack card flat" action={updateBomItem}><input type="hidden" name="id" value={b.id}/><label>Variation<SearchSelect name="variation_id" defaultValue={b.variation_id} placeholder="Type a variation" options={(variations || []).map((v: any) => ({ value: v.id, label: `${v.internal_sku} · ${v.variation_name}` }))} /></label><label>Part<SearchSelect name="part_id" defaultValue={b.part_id} placeholder="Type a part name or SKU" options={(parts || []).map((p: any) => ({ value: p.id, label: `${p.sku} · ${p.name}` }))} /></label><label>Qty<input name="quantity_per_unit" type="number" step="0.0001" defaultValue={b.quantity_per_unit}/></label><label>Notes<textarea name="notes" defaultValue={b.notes || ''}/></label><button type="submit">Save</button></form><form action={deleteBomItem}><input type="hidden" name="id" value={b.id}/><button className="danger small-btn" type="submit">Delete</button></form></details></td></tr>)}{(bomItems || []).length === 0 && <tr><td colSpan={5}><div className="empty-state">No BOM items for this product yet.</div></td></tr>}</tbody></table></div>
    </>
  )
}
