    ...
    <>
      ...
      <div className="card">
        ...
        <form className="stack" action={createBomItem}>
          <div className="form-row"><label>Variation<select name="variation_id" required><option value="">Choose variation</option>{(variations || []).map((v:any) => <option key={v.id} value={v.id}>{v.internal_sku} · {v.variation_name}</option>)}</select></label><label>Part<select name="part_id" required><option value="">Choose part</option>{(parts || []).map((p:any) => <option key={p.id} value={p.id}>{p.sku} · {p.name}</option>)}</select></label><label>Qty per unit<input name="quantity_per_unit" type="number" step="0.0001" required defaultValue="1" /></label></div>
          <label>Notes<textarea name="notes" /></label>
