...
export default async function ProductDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return (
    <>
      ...
      <div className="card table-card"><div className="table-head"><h2>Variations</h2></div><div className="wide-table"><table><thead><tr><th>Variation</th><th className="sku-col">Internal SKU</th><th>Status</th><th>Notes</th><th>Actions</th></tr></thead><tbody>{(variations || []).map((v:any) => <tr key={v.id}><td><span className="entity-name">{v.variation_name}</span><br/><span className="sku-small">{v.internal_sku}</span></td><td className="sku-col">{v.internal_sku}</td><td><span className={`badge ${v.active ? 'ok' : 'archived'}`}>{v.active ? 'active' : 'archived'}</span></td><td>{v.notes}</td><td><details><summary className="button small-btn secondary">Edit</summary><form className="stack card flat" action={updateVariation}><input type="hidden" name="id" value={v.id}/><input type="hidden" name="product_id" value={id}/><label>Variation<input name="variation_name" defaultValue={v.variation_name}/></label><label>Internal SKU<input name="internal_sku" defaultValue={v.internal_sku}/></label><label className="small"><span><input name="active" type="checkbox" defaultChecked={v.active} style={{ width:'auto', marginRight:8 }}/>Active</span></label><label>Notes<textarea name="notes" defaultValue={v.notes || ''}/></label><button type="submit">Save</button></form><form action={archiveVariation}><input type="hidden" name="id" value={v.id}/><input type="hidden" name="product_id" value={id}/><input type="hidden" name="active" value={v.active ? 'false' : 'true'}/><button className="danger small-btn" type="submit">{v.active ? 'Archive' : 'Restore'}</button></form></details></td></tr>)}{(variations || []).length === 0 && <tr><td colSpan={5}><div className="empty-state">No variations yet.</div></td></tr>}</tbody></table></div></div>

      <div className="card">
        <h2>Add BOM item</h2>
        <form className="stack" action={createBomItem}>
          <div className="form-row"><label>Variation<select name="variation_id" required><option value="">Choose variation</option>{(variations || []).map((v:any) => <option key={v.id} value={v.id}>{v.variation_name} · {v.internal_sku}</option>)}</select></label><label>Part<select name="part_id" required><option value="">Choose part</option>{(parts || []).map((p:any) => <option key={p.id} value={p.id}>{p.name} · {p.sku}</option>)}</select></label><label>Qty per unit<input name="quantity_per_unit" type="number" step="0.0001" required defaultValue="1" /></label></div>
          <label>Notes<textarea name="notes" /></label>
          <button type="submit">Add BOM item</button>
        </form>
      </div>

      <div className="card table-card"><div className="table-head"><h2>BOM recipe for this product</h2><Link className="button small-btn secondary" href="/boms">Full BOM sheet</Link></div><table><thead><tr><th>Variation</th><th>Part</th><th>Qty per unit</th><th>Notes</th><th>Actions</th></tr></thead><tbody>{(bomItems || []).map((b:any) => <tr key={b.id}><td><span className="entity-name">{b.product_variations?.variation_name}</span><br/><span className="sku-small">{b.product_variations?.internal_sku}</span></td><td><span className="entity-name">{b.parts?.name}</span><br/><span className="sku-small">{b.parts?.sku}</span></td><td>{num(b.quantity_per_unit)}</td><td>{b.notes}</td><td><details><summary className="button small-btn secondary">Edit</summary><form className="stack card flat" action={updateBomItem}><input type="hidden" name="id" value={b.id}/><label>Variation<select name="variation_id" defaultValue={b.variation_id}>{(variations || []).map((v:any)=><option key={v.id} value={v.id}>{v.variation_name} · {v.internal_sku}</option>)}</select></label><label>Part<select name="part_id" defaultValue={b.part_id}>{(parts || []).map((p:any)=><option key={p.id} value={p.id}>{p.name} · {p.sku}</option>)}</select></label><label>Qty<input name="quantity_per_unit" type="number" step="0.0001" defaultValue={b.quantity_per_unit}/></label><label>Notes<textarea name="notes" defaultValue={b.notes || ''}/></label><button type="submit">Save</button></form><form action={deleteBomItem}><input type="hidden" name="id" value={b.id}/><button className="danger small-btn" type="submit">Delete</button></form></details></td></tr>)}{(bomItems || []).length === 0 && <tr><td colSpan={5}><div className="empty-state">No BOM items for this product yet.</div></td></tr>}</tbody></table></div>
    </>
  )
}
