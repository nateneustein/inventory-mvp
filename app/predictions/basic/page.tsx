...
export default async function BasicPredictionPage({ searchParams }: { searchParams?: Promise<{ q?: string, status?: string, zoom?: string }> }) {
  ...
  return (
    <>
      ...
      <div className="card spreadsheet-toolbar"><strong>Spreadsheet tools</strong><div className="zoom-controls big"><span>Zoom</span>{['80','90','100','110','125','150'].map(z => <Link key={z} className={`button small-btn ${zoom === z ? '' : 'secondary'}`} href={predictionHref(params, z)}>{z}%</Link>)}</div><span className="muted small">The header row and first column stay sticky while you scroll inside the sheet.</span></div>
