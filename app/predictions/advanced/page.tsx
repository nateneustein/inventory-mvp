...
export default async function AdvancedPredictionPage({ searchParams }: { searchParams?: Promise<{ part_id?: string }> }) {
  ...
  const { data: movements } = selectedPartId ? await supabase.from('inventory_movements').select('*').eq('part_id', selectedPartId).lt('quantity', 0).gte('created_at', daysAgo(380)).order('created_at', { ascending: false }) : { data: [] as any[] }
  ...
  for (let i = 0; i < weekValues.length; i++) { largestAbsoluteWeek = Math.max(largestAbsoluteWeek, weekValues[i]); if (i > 0 && weekValues[i] > 0) { const prev = weekValues[i - 1]; if (prev > 0) largestPercentJump = Math.max(largestPercentJump, (weekValues[i] - prev) / prev) } }

  const leadTime = Number(selected?.lead_time_days_max || 0), bufferDays = Number(selected?.safety_stock_days || 0), incoming = Number(selected?.incoming_qty || 0), onHand = Number(selected?.on_hand || 0)
  ...
  return (
    <>
      <div className="page-head"><div><h1>Advanced Prediction</h1><p className="muted">Calculate multiple reorder levels from usage, lead time, buffer, spikes, and last-year comparison.</p></div><Link className="button secondary" href="/predictions/basic">Basic sheet</Link></div>
      <div className="card"><form className="filter-bar"><label>Part<select name="part_id" defaultValue={selectedPartId || ''}>{(parts || []).map((p:any)=><option key={p.part_id} value={p.part_id}>{p.name} · {p.sku}</option>)}</select></label><button type="submit">Calculate</button>{selected && <Link className="button ghost" href={`/parts/${selected.part_id}`}>Open part</Link>}</form></div>
      {selected && <>
        <div className="grid"><div className="card kpi-card"><div className="muted">Current stock</div><div className="kpi">{num(onHand)}</div></div><div className="card kpi-card"><div className="muted">Incoming</div><div className="kpi">{num(incoming)}</div></div><div className="card kpi-card"><div className="muted">Lead time + buffer</div><div className="kpi">{num(coverageDays)} days</div></div><div className="card kpi-card"><div className="muted">Highest daily estimate</div><div className="kpi">{num(Math.max(highDaily, seasonalDaily))}</div></div></div>
        ...
        <div className="card table-card"><div className="table-head"><h2>Why</h2></div><table><tbody><tr><th>Last 7 days usage</th><td>{num(d7)}</td></tr><tr><th>Last 30 days usage</th><td>{num(d30)}</td></tr><tr><th>Last 90 days usage</th><td>{num(d90)}</td></tr><tr><th>Same-ish period last year</th><td>{num(lastYearComparable)}</td></tr><tr><th>Largest weekly usage seen</th><td>{num(largestAbsoluteWeek)}</td></tr><tr><th>Largest percentage jump seen</th><td>{num(largestPercentJump * 100)}%</td></tr><tr><th>Manual reorder point</th><td>{num(selected.reorder_point)}</td></tr><tr><th>Target stock</th><td>{num(selected.target_stock)}</td></tr></tbody></table></div>
      </>}
    </>
  )
}
