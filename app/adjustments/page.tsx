    ...
    <>
      ...
      <div className="grid">
        <div className="card">
          ...
          <form className="stack" action={createManualAdjustment}>
            <label>Part<select name="part_id" required><option value="">Choose part</option>{(parts || []).map((p: any) => <option key={p.id} value={p.id}>{p.name} · {p.sku}</option>)}</select></label>
        ...
        <div className="card">
          ...
          <form className="stack" action={createInventorySwitch}>
            <label>Original part demanded, optional<select name="from_part_id"><option value="">Choose original part</option>{(parts || []).map((p: any) => <option key={p.id} value={p.id}>{p.name} · {p.sku}</option>)}</select></label>
            <label>Actual part used<select name="to_part_id" required><option value="">Choose actual part used</option>{(parts || []).map((p: any) => <option key={p.id} value={p.id}>{p.name} · {p.sku}</option>)}</select></label>
      ...
      <div className="card wide-table">
        ...
        <table>
          ...
          <tbody>
            {(movements || []).map((m: any) => <tr key={m.id}><td>{date(m.created_at)}</td><td><span className="entity-name">{m.parts?.name}</span><br/><span className="sku-small">{m.parts?.sku}</span></td><td>{m.movement_type}</td><td>{num(m.quantity)}</td><td>{m.reason}</td><td>{m.notes}</td></tr>)}
